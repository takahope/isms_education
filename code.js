...
function debugCurrentUserAssignmentClassification(emailOverride) {
  const fallbackEmail = getCurrentUserEmail();
  const targetEmail = normalizeEmail_(emailOverride || fallbackEmail);

  Logger.log('=== 職務判定診斷開始 ===');
  Logger.log(`targetEmail: ${targetEmail || '(empty)'}`);
  Logger.log(`sessionEmail: ${normalizeEmail_(fallbackEmail) || '(empty)'}`);

  try {
    if (!targetEmail) {
      throw new Error('無法取得目標信箱，請確認 Session.getActiveUser() 是否可用，或手動傳入 email。');
    }

    const masterSS = getMasterSpreadsheet_();
    const personnelSheet = getRequiredSheet_(masterSS, '人員主檔');
    const assignmentSheet = getRequiredSheet_(masterSS, '人員職務配置');
    const name = findPersonnelNameByEmail_(personnelSheet, targetEmail);
    const assignments = getAssignmentsByEmail_(assignmentSheet, targetEmail);
    const primaryMode = assignments.length > 0 ? getPrimaryAssignmentMode_(assignments) : null;
    const primaryAssignments = isExplicitPrimaryKind_(primaryMode)
      ? assignments.filter((item) => classifyAssignmentKind_(item.orgCode) === primaryMode)
      : [];
    ...
    const assignmentTypeMap = assignments.length > 0 ? buildAssignmentTypeMap_(assignments) : new Map();

    const assignmentDiagnostics = assignments.map((assignment) => {
      const itemKind = classifyAssignmentKind_(assignment.orgCode);
      const managerEmail = normalizeEmail_(assignment.managerEmail);
      return {
        ...
        managerEmail,
        itemKind,
        resolvedType: assignmentTypeMap.get(getAssignmentIdentityKey_(assignment)) || '兼任',
        matchesPrimaryManager: primaryManagerEmails.includes(managerEmail)
      };
    });
    ...
    const summary = {
      success: true,
      targetEmail,
      sessionEmail: normalizeEmail_(fallbackEmail),
      foundName: name,
      personnelFound: Boolean(name),
      ...
      primaryAssignments: primaryAssignments.map((assignment) => ({
        ...
        orgName: String(assignment.orgName || '').trim(),
        title: String(assignment.title || '').trim(),
        managerEmail: normalizeEmail_(assignment.managerEmail),
        itemKind: classifyAssignmentKind_(assignment.orgCode)
      })),
      assignments: assignmentDiagnostics
    };

    logAssignmentDebugBlock_('人員主檔診斷', {
      targetEmail,
      foundName: name,
      personnelFound: Boolean(name)
    });
    logAssignmentDebugBlock_('主職判定摘要', {
      assignmentCount: assignments.length,
      primaryMode,
      primaryManagerEmails
    });
    logAssignmentDebugBlock_('主職候選資料', summary.primaryAssignments);
    logAssignmentDebugBlock_('逐筆職務判定結果', assignmentDiagnostics);
    Logger.log('=== 職務判定診斷結束 ===');

    return summary;
  } catch (error) {
    const message = error && error.message ? error.message : String(error);
    Logger.log(`職務判定診斷失敗: ${message}`);
...
function debugAssignmentRowsByKeyword() {
  return debugAssignmentRowsForEmail_('itsuw', 'TF-GRP-AUDIT', '內部稽核執行小組');
}

function debugAssignmentRowsForEmail_(emailOverride, orgCodeKeyword, orgNameKeyword) {
  const targetEmail = normalizeEmail_(emailOverride);
  const normalizedOrgCodeKeyword = String(orgCodeKeyword || '').trim().toUpperCase();
  const normalizedOrgNameKeyword = String(orgNameKeyword || '').trim();

  Logger.log('=== 職務列掃描診斷開始 ===');
  Logger.log(`targetEmail: ${targetEmail || '(empty)'}`);
  Logger.log(`orgCodeKeyword: ${normalizedOrgCodeKeyword || '(empty)'}`);
  Logger.log(`orgNameKeyword: ${normalizedOrgNameKeyword || '(empty)'}`);

  try {
    const masterSS = getMasterSpreadsheet_();
    const assignmentSheet = getRequiredSheet_(masterSS, '人員職務配置');
    const rows = assignmentSheet.getDataRange().getDisplayValues();
    const matchedRows = [];

    for (let i = 1; i < rows.length; i += 1) {
      const rawEmail = String(rows[i][0] || '');
      const normalizedEmail = normalizeEmail_(rawEmail);
      const orgCode = String(rows[i][2] || '').trim();
      const orgName = String(rows[i][3] || '').trim();
      const matches = (
        (targetEmail && normalizedEmail.includes(targetEmail))
        || (normalizedOrgCodeKeyword && String(orgCode || '').trim().toUpperCase().includes(normalizedOrgCodeKeyword))
        || (normalizedOrgNameKeyword && orgName.includes(normalizedOrgNameKeyword))
      );
      ...
      const simulatedAssignments = targetEmail ? getAssignmentsByEmail_(assignmentSheet, targetEmail).slice() : [];
      const existsInCollectedSet = simulatedAssignments.some((item) => item.rowIndex === assignment.rowIndex);
      if (targetEmail && !existsInCollectedSet && normalizedEmail === targetEmail) {
        simulatedAssignments.push(assignment);
      }
      const simulatedTypeMap = simulatedAssignments.length > 0 ? buildAssignmentTypeMap_(simulatedAssignments) : new Map();
      ...
      matchedRows.push({
        ...
        emailCharCodes: toCharCodes_(rawEmail),
        emailDescription: describeStringForDebug_(rawEmail),
        emailExactMatch: rawEmail === targetEmail,
        emailTrimMatch: String(rawEmail || '').trim() === targetEmail,
        emailNormalizedMatch: normalizedEmail === targetEmail,
        wouldBeCollected: normalizedEmail === targetEmail,
        name: String(assignment.name || '').trim(),
        orgCode: String(assignment.orgCode || '').trim(),
        orgName: String(assignment.orgName || '').trim(),
        title: String(assignment.title || '').trim(),
        managerEmail: String(assignment.managerEmail || '').trim(),
        itemKind: classifyAssignmentKind_(assignment.orgCode),
        simulatedResolvedType: simulatedTypeMap.get(getAssignmentIdentityKey_(assignment)) || null
      });
    }
    ...
    const summary = {
      success: true,
      targetEmail,
      orgCodeKeyword: normalizedOrgCodeKeyword,
      orgNameKeyword: normalizedOrgNameKeyword,
      matchedRowCount: matchedRows.length,
      matchedRows
    };

    logAssignmentDebugBlock_('職務列掃描摘要', {
      targetEmail,
      orgCodeKeyword: normalizedOrgCodeKeyword,
      orgNameKeyword: normalizedOrgNameKeyword,
      matchedRowCount: matchedRows.length
    });
    logAssignmentDebugBlock_('命中原始列', matchedRows);
    Logger.log('=== 職務列掃描診斷結束 ===');

    return summary;
  } catch (error) {
    const message = error && error.message ? error.message : String(error);
    Logger.log(`職務列掃描診斷失敗: ${message}`);
    if (error && error.stack) Logger.log(error.stack);
    Logger.log('=== 職務列掃描診斷結束 ===');
    return {
      success: false,
...
function logAssignmentDebugBlock_(title, payload) {
  Logger.log(`--- ${title} ---`);
  Logger.log(JSON.stringify(payload, null, 2));
}

function toCharCodes_(value) {
  return String(value || '').split('').map((char) => char.charCodeAt(0));
}

function describeStringForDebug_(value) {
  return String(value || '')
    .split('')
    .map((char) => `${char || '(empty)'}:${char.charCodeAt(0)}`)
    .join(' | ');
}
...
/**
   * ==============================
   * 職務判定測試入口
   * Apps Script 的 Run 只能執行無參數函式
   * ==============================
   */

  // 測目前登入帳號
  function debugMyAssignmentClassification() {
    return debugCurrentUserAssignmentClassification();
  }

  // 測指定帳號
  function debugSpecificUserAssignmentClassification() {
    return debugCurrentUserAssignmentClassification('bio.tw');
  }
