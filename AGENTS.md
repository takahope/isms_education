...
- 2026-09-11: Implemented Option A for shadow recipient login reminders. Added `buildNotificationShadowLoginReminderHtml_()` in `code.js` and updated `injectCapabilityUrlIntoHtmlBody_` to dynamically replace standard Google Workspace login reminders with a dedicated passwordless authentication prompt ("※ 專屬免登入提醒：此專屬連結已內嵌安全認證憑證，點擊進入後系統將自動認列您的公務身分，無需切換或登入 Google 組織帳號..."). Official recipients retain standard login reminders. Updated unit tests in `test/mention-shadow-capability-dispatch.test.js` and `test/debug-leader-profile.test.js`, and confirmed all 40 test suites pass.
