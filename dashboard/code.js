...
function buildStationManagerAnnouncementTemplate_(courseTitle) {
  return {
    ...
    htmlBody: [
      '<p>{{駐站管理稱呼}}</p>',
      `<p>因應本次<strong>${escapeHtml_(courseTitle || '資訊安全暨個資保護')}教育訓練課程</strong>作業安排，教育訓練管理工具將自動追蹤收案人員上課情形，並依系統資料自動通知發信。</p>`,
      '<p>為了讓後續通知更準確，建議您在上課前先協助確認您所管理的駐站與收案人員資訊是否正確；若資料有不一致、成員歸屬錯誤，或管理關係需要調整，也請一併協助更新。</p>',
      '<p>本次教育訓練採分階段通知，系統會依照駐站管理員維護的駐站資訊，持續安排後續教育訓練通知。若資料已確認正確，後續通知將由系統接續處理，您不需要另外協助轉發信件至駐站。</p>',
